---
endpoint: "GET /business/payments/v1/product_orders/{product_order_uid}"
domain: sales
tags: [product_orders]
...
Retrieves a single product order by its UID. Requires a prerequisite step to create a product order first since there is no list endpoint for product orders. The endpoint works via the fallback API with a staff token.
...
steps:
  - id: get_products
    description: "Fetch products to get a product ID for order creation"
    method: GET
...
steps:
  - id: main_request
    description: "Get a single product order by UID"
    method: GET
