...
| type | Type of the offering. Possible values include: **package** for the main business subscription; **app** for marketplace app subscriptions and **addon** for other SKUs like SMS and staff_seats. **one_time_purchase** for single-charge packs such as an SMS or token bundle. A user can have one subscription with package, one subscription per SKU of type apps and any number of subscriptions for any SKU | string (enum: `package`, `app`, `addon`, `one_time_purchase`) | Yes |
...
| quotas | The credit the offering grants and the window it is granted over. Always present: an offering created without an explicit quota carries its own `quantity` with a null `period`, meaning the credit is not metered here. `period` is `monthly` (requires an `addon` paid by `bundle`) or `life_long` (requires a `one_time_purchase` paid by `single_charge`/`external_single_charge`). `quantity` is the credit; `-1` means unlimited. Written with the offering and never changed once a period is set. Exactly one entry. | array of objects | Yes |
